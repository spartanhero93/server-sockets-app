...
module.exports = (client, factory) => {
  ...
  FactoryModel.findOne({ _id }, (err, foundFactory) => {
    if (err) {
      return client.emit(
        'Error',
        'Error has been found while trying to search for the factory'
      )
    }
    if (!foundFactory) return client.emit('Error', 'Factory does not exist')
    ...
    return newFactory.save((saveError, updatedFactory) => {
      saveError
        ? client.emit('Error', 'Error occurred while updating factory')
        : client.emit('updateFactory', updatedFactory)
    })
  })
}
